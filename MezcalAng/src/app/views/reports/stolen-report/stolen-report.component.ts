...
@Component({
  selector: 'app-stolen-report',
  templateUrl: './stolen-report.component.html',
  styleUrls: ['./stolen-report.component.scss']
})
export class StolenReportComponent implements OnInit {
  ...
  constructor(private excelService: ExcelService,private _toastr: ToastrService,private fb: FormBuilder, private apiService: ApiService, private toastrService: ToastrService) {
     this.SetDate();
   }
   search()
   {
  ...
  // get all stolen hologram stock from the api.
  public getAllStolenStock(event: any): any {
    this.apiService.postApi( 'Stolen/GetAllStolenStock',event).subscribe((res: any) => {
...
//////// Get Stolen  List for Excel

getLazyLoadLblPrintForExport(event: any) {
  this.apiService.postApi( 'Stolen/GetAllStolenStock',event).subscribe((response: any) => {
    ...
    if (response.responseCode == 200) {
      ...
      this.ExcelReportResult.slice();
        ...
        this.excelService.exportAsExcelFile(this.ExcelReportResult, 'Reporte de Producto Robado - ' + new Date());


    }
    else if (response.responseCode == 400){

      this._toastr.warning('', response.responseMessage);
    }
    else {
      this.list = response.ResponseData;
      this.totalRecords = 0;
